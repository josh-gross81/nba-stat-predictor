...
evaluation_results = pd.DataFrame({
    'Target Variable': target_columns,
...
# Save the trained model to a file so it can be used in the Flask app
joblib.dump(model, 'nba_model.pkl')  # Save the model as 'nba_model.pkl'
print("Model has been saved as 'nba_model.pkl'.")

# Make predictions for all players
y_pred = model.predict(X)

# Add player names back to the predictions
player_names = merged_data_clean['Name'].values  # Use names from the clean dataset
...
output_file_path = '/Users/joshgross/Downloads/nba_predicted_stats_ranked_by_ppg.txt'
with open(output_file_path, 'w') as f:
    f.write(predicted_stats_sorted.to_string(index=False))

print(f"Predicted stats for all players have been exported to {output_file_path}")
